  ...
  <div>
    ...
    <div v-for="item in list" :key="item.id" class="flex mt-5">
      ...
      <div class="flex-1 pb-5 ml-4 border-0 border-b border-gray-200 dark:border-gray-200/20 border-solid">
        <div>
          <span class="text-cyan-600"> {{ item.user.nickname }}：</span>
          <span>{{ item.content }}</span>
        </div>
        <div v-if="item.beReplied.length" class="p-2 mt-2 bg-stone-100 dark:bg-stone-100/20 rounded-md">
          <span class="text-cyan-600">{{ item.beReplied[0].user.nickname }}：</span>
